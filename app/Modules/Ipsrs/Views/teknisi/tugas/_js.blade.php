<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit', '#form-log-kerja', function(e) {
            ...
            if (navParam) {
                ...
                console.warn(
                    "Peringatan: Parameter 'n' tidak ditemukan. Ini mungkin gagal di middleware.");
            }
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (responseData.code == '01' || responseData.code == '02') {
                        _toast('success', responseData.message ||
    ...
    // Handler untuk tombol scan barcode
    $(document).off('click', '.btn-scan-barcode').on('click', '.btn-scan-barcode', function(e) {
        e.preventDefault();
        const orderKerjaId = $(this).data('order-kerja-id');
        const n = $(this).data('n') || '';

        // Buat URL yang benar dengan parameter yang tepat
        const url = _base_url + 'ipsrs/teknisitugas/form_scan_modal/' + orderKerjaId;
        const modalUrl = url + (n ? '?n=' + n : '');

        // Buka modal dengan URL yang benar
        _modal(null, {
            uri: modalUrl,
            size: 'modal-lg',
    ...
    // Tambahkan handler untuk form log kerja
    $(document).on('submit', '#form-log-kerja', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                _toast(isSuccess ? 'success' : 'error', res.message || (isSuccess ? 'Data berhasil disimpan' : 'Terjadi kesalahan'));
                
                if (isSuccess) {
                    // Tutup modal
                    $('#log-kerja-modal').modal('hide');
