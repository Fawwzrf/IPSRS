...
class LaporanModel extends Model
{
    ...
    public function getDatatablesKinerjaAset($filter, $start, $length, $order, $search)
    {
        ...
        // Order
        $columns = [
            'a.asset_nm', 'l.lokasi_nm', 'jumlah_ok',
            'jumlah_perbaikan', 'jumlah_pemeliharaan', 'terakhir_ditangani'
        ];
        ...
        // Count total
        $sqlCount = "SELECT COUNT(*) as total FROM mst_asset a WHERE a.deleted_st = 0";
        $total = DbModel::rawData('row_array', $sqlCount)['total'] ?? 0;

        // Count filtered
        $sqlFiltered = "SELECT COUNT(DISTINCT a.asset_id) as filtered
                FROM mst_asset a
                ...
                WHERE {$whereClause}";
        $filtered = DbModel::rawData('row_array', $sqlFiltered, $bindings)['filtered'] ?? 0;
    ...
    public function getDatatablesKinerjaTeknisi($filter, $start, $length, $order, $search)
    {
        ...
        // Count filtered
        $sqlFiltered = "SELECT COUNT(DISTINCT p.pegawai_id) as filtered
                FROM mst_pegawai p
        ...
        $mappedData = [];
        foreach ($data as $row) {
            $persentase = ($row['total_tugas'] > 0)
                ? round(($row['tugas_selesai'] / $row['total_tugas']) * 100, 2)
                : 0;
            $mappedData[] = [
    ...
    public function getDatatablesBiayaPemeliharaan($filter, $start, $length, $order, $search)
    {
        ...
        $columns = [
            'ok.tgl_dibuat', 'ok.order_kerja_id', 'asset_nm',
            'ok.jenis', 'total_biaya_sparepart', 'biaya_lain', 'total_biaya_ok'
        ];
        ...
        // Count filtered
        $sqlFiltered = "SELECT COUNT(DISTINCT ok.order_kerja_id) as filtered
    ...
    public function getDatatablesKinerjaTim($filter, $start, $length, $order, $search)
    {
        ...
        $columns = [
            'ok.order_kerja_id', 'ok.jenis', 'p.pegawai_nm', 'nama_aset',
            'durasi_respon_admin', 'durasi_penerimaan_teknisi', 'durasi_pengerjaan', 'durasi_total'
        ];
        ...
        // Count filtered
        $sqlFiltered = "SELECT COUNT(DISTINCT pt.penugasan_id) as filtered
